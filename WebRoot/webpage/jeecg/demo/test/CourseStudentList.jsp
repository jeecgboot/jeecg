...
<table border="0" cellpadding="2" cellspacing="0" id="jeecgOrderCustom_table">
	...
	<c:if test="${fn:length(studentsList)  <= 0 }">
			<tr>
				...
				<td align="left">
				 <t:dictSelect field="students[0].sex" typeGroupCode="sex" hasLabel="false" defaultVal="${jgDemo.sex}"></t:dictSelect>
				</td>
   			</tr>
	</c:if>
	<c:if test="${fn:length(studentsList)  > 0 }">
		<c:forEach items="${studentsList}" var="poVal" varStatus="stuts">
			<tr>
				...
				<td align="left">
				<t:dictSelect field="students[${stuts.index }].sex" typeGroupCode="sex" hasLabel="false" defaultVal="${poVal.sex}"></t:dictSelect>
				</td>
   			</tr>
		</c:forEach>
	</c:if>	
