 ...
 <script type="text/javascript">
  $(document).ready(function(){
 		...
 		<#if (po.showType?index_of("datetime")!=-1 || po.showType?index_of("date")!=-1) && po.queryMode=="single">
 			$("#${entityName?uncap_first}Listtb").find("input[name='${po.fieldName}']").attr("class","easyui-${po.showType}box");
 		<#else>
 		<#if (po.showType?index_of("datetime")!=-1 || po.showType?index_of("date")!=-1) && po.queryMode=="group">
 			$("#${entityName?uncap_first}Listtb").find("input[name='${po.fieldName}_begin']").attr("class","easyui-${po.showType}box");
 			$("#${entityName?uncap_first}Listtb").find("input[name='${po.fieldName}_end']").attr("class","easyui-${po.showType}box");
 		</#if>
 		</#if>
		</#list>
 });
