...
public class ${entityName}Controller extends BaseController {
	...
	@ResponseBody
	public AjaxJson doAdd(HttpServletRequest request,String formdata) {
		AjaxJson j = new AjaxJson();
		String message = "添加成功";
		${entityName}Page page = JSON.parseObject(formdata, ${entityName}Page.class);
		<#list subTab as sub>
		List<${sub.entityName}Entity> ${sub.entityName?uncap_first}List = page.get${sub.entityName}List();
		</#list>
		${entityName}Entity entity = new ${entityName}Entity();
		try {
			MyBeanUtils.copyBeanNotNull2Bean(page, entity);
			${entityName?uncap_first}Service.addMain(entity, <#list subTab as sub>${sub.entityName?uncap_first}List<#if sub_has_next>,</#if></#list>);
	...
	@ResponseBody
	public AjaxJson doUpdate(HttpServletRequest request,String formdata) {
		AjaxJson j = new AjaxJson();
		String message = "更新成功";
		${entityName}Page ${entityName?uncap_first}Page = JSON.parseObject(formdata, ${entityName}Page.class);
		try{
			${entityName?uncap_first}Service.updateMain(${entityName?uncap_first}Page);
	...
	public ModelAndView goAdd(${entityName}Entity ${entityName?uncap_first}, HttpServletRequest req) {
		...
		req.setAttribute("${entityName?uncap_first}Page",new  ${entityName}Page());
	...
	public ModelAndView goUpdate(HttpServletRequest req) {
		...
		if (StringUtil.isNotEmpty(id)) {
			req.setAttribute("formId", id);
		}
		return new ModelAndView("${bussiPackage?replace(".","/")}/${entityPackage}/${entityName?uncap_first}-update");
	}
	
	
	@RequestMapping(params = "initFormData")
	@ResponseBody
	public AjaxJson initFormData(HttpServletRequest request) {
		String id = request.getParameter("id");
		AjaxJson j = new AjaxJson();
		${entityName}Page ${entityName?uncap_first}Page = new ${entityName}Page();
		if(oConvertUtils.isEmpty(id)){
			<#list subTab as sub>
				List<${sub.entityName}Entity> ${sub.entityName?uncap_first}EntityList  = new ArrayList<${sub.entityName}Entity>();
				${sub.entityName?uncap_first}EntityList.add(new ${sub.entityName}Entity());
				${entityName?uncap_first}Page.set${sub.entityName}List(${sub.entityName?uncap_first}EntityList);
			</#list>
		}else{
			${entityName}Entity ${entityName?uncap_first} = ${entityName?uncap_first}Service.getEntity(${entityName}Entity.class,id);
			try {
		    ...
		    if(${sub.entityName?uncap_first}EntityList==null || ${sub.entityName?uncap_first}EntityList.size()<=0){
		    	${sub.entityName?uncap_first}EntityList = new ArrayList<${sub.entityName}Entity>();
		    	${sub.entityName?uncap_first}EntityList.add(new ${sub.entityName}Entity());
		    }
		    ${entityName?uncap_first}Page.set${sub.entityName}List(${sub.entityName?uncap_first}EntityList);
		...
		j.setObj(${entityName?uncap_first}Page);
		return j;
	}
