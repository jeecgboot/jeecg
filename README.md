...
* 7.集成工作流activiti，并实现在线画流程、在线配置表单、流程串表单无需编码，强大的OA自动化能力，极大的简化bpm工作流的开发；另外通过扩展接口编写业务，方便业务扩展；
* 7.工作流的实现，实现了各种中国国情的功能，任意跳转、任意驳回、任意委派等等；流程节点支持配置个性化表单；同时web端审批与移动端审批保持同步进行，一次配置双点审批。
